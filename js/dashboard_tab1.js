...
document.getElementById("anio")?.addEventListener("change", cargarReporte);
document.getElementById("mes")?.addEventListener("change", cargarReporte);
document.getElementById("trabajador")?.addEventListener("change", cargarReporte);
...
function cargarReporte() {
    ...
    fetch(
        ...
        .then(data => {
            ...
            // COMISIONES DE TRABAJADORES (ROL 3)
            let comisionYape = 0;
            let comisionPlin = 0;
            let comisionEfectivo = 0;
            ...
            data.forEach(item => {
                ...
                // Calcular comisión para trabajadores con rol 3
                if (
                    item.tipo_servicio === "i" &&
                    parseInt(item.rol) === 3
                ) {
                    ...
                    const comision =
                        montoNumero * porcentaje / 100;

                    switch (item.tipo_pago) {

                        case "y":
                            comisionYape += comision;
                            break;

                        case "p":
                            comisionPlin += comision;
                            break;
            ...
            // Neto real por método de pago
            const netoRealYape =
                netoYape - comisionYape;

            const netoRealPlin =
                netoPlin - comisionPlin;

            const netoRealEfectivo =
                netoEfectivo - comisionEfectivo;

            const netoRealTarjeta =
                netoTarjeta - comisionTarjeta;
            ...
            // PINTAR NETOS
            pintarMonto("netoYape", netoYape);
            pintarMonto("netoPlin", netoPlin);
            pintarMonto("netoEfectivo", netoEfectivo);
            pintarMonto("netoTarjeta", netoTarjeta);
            pintarMonto("totalBruto", totalBruto);
            pintarMonto("totalNeto", totalNeto);
        })
        .catch(console.error);
}
